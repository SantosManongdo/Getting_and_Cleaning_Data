...
# unzip the features file to the temporary directory
...
# combine the measurement data in train and test
data_X <- rbind(x_train, x_test)
# combine the activity data in train and test
data_Y <- rbind(y_train, y_test)
# combine the subject idetifier data in train and test
...
# Note the dataset in step 1 does not contain any column headings yet. For the measurement data, i.e  data_X, the column headings are in features.txt
# load the features names first
features <- fread(fpath2,sep = " ", header = F)

# subset features with mean and std in the name 
mean_sdev_names <- c("mean","std")
# get the feature names
mean_sdev_features <- grep(mean_sdev_names, features$V2)
# using selected feature index with mean and std in feature names, subset data_X
mean_sdev_dframe <- data_X %>% select(all_of(mean_sdev_features))
# add column names
names(mean_sdev_dframe) <- features$V2[mean_sdev_features]
...
# First we clean the feature names replacing the characters comma and hyphen with underscore and open/close parentheses with null
...
# we know that there are column names that have exactly the same names. BandEnergy features were recorded for each direction X,Y,Z but the column name used for each measurement by direction were the same. Therefore, duplicate columns are present.
# To confirm there are duplicate column names, we use the duplicated() function.
duplicated(features$V2)

# Te make the column names unique to have unique column names
features$V2 <- make.unique(features$V2) 

# Now that feature names are cleaned, we can appropriately name the dataset.
